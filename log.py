...
def record_turn(
    ...
    if turn == 0:
        with open(output_file, mode="w") as file:
            file.write(
                "Step,PlayerLevel,ActiveBeat#,ActiveBeatName,ZoneLevel,PowerRatio,Ability,Difficulty,BeatType,RandCat,OutcomeCategory,SkillDifficulty,Success?,RepDelta,XP_Earned,Gold_Earned,DropID,SuccessChanceCombat,SkillCheckProbability,DeathChance,Death?,RepairCost,Respec?,RespecCost,VendorTaxPct,Gold_Spent,NetGoldChange,CumulativeGold,CumulativeXP,CumulativeRep,Eq_Weapon,Eq_Chest,Eq_Helm,Eq_Legs,Eq_Accessory,GearScore,CatStatKey,CategoryDC,BeatDC_lookup,BaseStat,PerLevel,StatScore,SuccessChance_NonCombat\n"
            )
    ...
    with open(output_file, mode="a") as file:
        file.write(
            f"{turn + 1},{player.level},{world.BeatNum},{world.BeatName},{world.ZoneLevel},{stats.Power_Ratio:.3f},{stats.Ability:.3f},{stats.Difficulty:.3f},BeatType,RandCat,{stats.OutcomeCategory},{stats.SkillDifficulty},{stats.Success},{stats.RepDelta},{stats.XP_Earned},{stats.Gold_Earned},{stats.DropID},{stats.SuccessChanceCombat:.2f},{stats.SkillCheckProbability:.2f},{stats.DeathChance:.2f},{stats.Death},RepairCost,Respec?,RespecCost,VendorTaxPct,{stats.Gold_Spent},{stats.Gold_Earned - stats.Gold_Spent},{player.gold},{player.culumative_exp()},CumulativeRep,{player.equipment.weapon},{player.equipment.chest},{player.equipment.helm},{player.equipment.legs},{player.equipment.accessory},{player.equipment.get_score()},{stats.CatStatKey},{stats.CategoryDC},{world.BeatDC},{stats.BaseStat},{stats.PerLevel},{stats.StatScore:.3f},{stats.SuccessChance_NonCombat:.2f}\n"
        )
